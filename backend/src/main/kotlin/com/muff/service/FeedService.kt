...
class FeedService {
    ...
    fun getSources(): List<SourceResponse> = transaction {
        Sources.selectAll()
    ...
    fun getNewFeed(cursor: FeedCursor?, limit: Int): FeedResponse = transaction {
        val query = Articles.innerJoin(Sources, { Articles.sourceId }, { Sources.sourceId })
            .selectAll()
    ...
    fun getCategoryFeed(category: String, cursor: FeedCursor?, limit: Int): FeedResponse = transaction {
        val query = Articles.innerJoin(Sources, { Articles.sourceId }, { Sources.sourceId })
            .selectAll()
    ...
    fun getPopularFeed(limit: Int): FeedResponse = transaction {
        val latestBucket = PopularityBuckets
    ...
    fun getAllSources(): List<SourceResponse> = transaction {
        Sources.selectAll()
            .orderBy(Sources.name to SortOrder.ASC)
            .map { it.toSourceResponse() }
    }

    fun createSource(req: CreateSourceRequest): SourceResponse = transaction {
    ...
    fun updateSource(id: String, req: UpdateSourceRequest): SourceResponse? = transaction {
        val uuid = UUID.fromString(id)
        val updated = Sources.update({ Sources.sourceId eq uuid }) {
            it[name] = req.name
            it[rssUrl] = req.rssUrl
            it[siteUrl] = req.siteUrl
            it[defaultCategory] = req.defaultCategory
    ...
    fun deleteSource(id: String): Boolean = transaction {
        val uuid = UUID.fromString(id)
    ...
    private fun buildFeedResponse(rows: List<ResultRow>, limit: Int): FeedResponse {
