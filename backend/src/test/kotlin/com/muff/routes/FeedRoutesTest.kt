...
class FeedRoutesTest {
    ...
    @Test
    fun `GET feed new returns articles in descending order`() = testApplication {
        application {
    ...
    @Test
    fun `GET feed new with cursor paginates correctly`() = testApplication {
        application {
            configureSerialization()
            configureStatusPages()
            configureRouting(FeedService())
    ...
    @Test
    fun `GET feed category filters by category`() = testApplication {
        application {
            configureSerialization()
            configureStatusPages()
            configureRouting(FeedService())
    ...
    @Test
    fun `GET feed new with invalid cursor returns 400`() = testApplication {
        application {
            configureSerialization()
            configureStatusPages()
            configureRouting(FeedService())
    ...
    @Test
    fun `GET feed popular returns empty when no buckets`() = testApplication {
        application {
            configureSerialization()
            configureStatusPages()
            configureRouting(FeedService())
